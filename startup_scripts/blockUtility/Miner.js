 ...
 */
function miner(BlockName ,OffsetScheme,BlockIdScheme,itemLoot,itemSuccess,rfTick,rfMaxCap){
        ...
        if (state.persistentData.getBoolean("assembled")) {
          if (rnd(0, itemSuccess) == itemSuccess) {
            if (upface.inventory != null) {
              upface.inventory.insertItem(itemLoot[rnd(0, itemLoot.length)], false);
             } 
            else {
                state.level.spawnParticles(
                    "minecraft:smoke",
      ...
      be.attachCapability(
        CapabilityBuilder.ENERGY.customBlockEntity()
          ...
          .getEnergyStored((energy) => {
            return energy.persistentData.getInt("amount") > rfMaxCap ? 1000 : energy.persistentData.getInt("amount") ;
          })
