BlockEvents.rightClicked("kubejs:core", (event) => {
  ...
  for ([a, b, c] of scheme) {
    if (block.offset(a, b, c).id !== "supplementaries:pedestal") {
      server.runCommandSilent(
        `particle dust 1 0 0 10 ${x + a} ${y + b + 0.5} ${
          z + c
        } 0 0 0 0 1 force`
      );
    } else {
      if (!block.offset(a, b, c).inventory.isEmpty()) {
        items_list.push(
          Utils.id(block.offset(a, b, c).inventory.allItems[0].id)
        );
  ...
  const recipecollection = [
    {
      input: [
        "minecraft:stone",
        "minecraft:granite", //I NEED TO FIX THIS BUT NOT TODAY!
        "minecraft:sand",
        "minecraft:dirt",
      ],
  ...
  recipecollection.forEach((recipe_id) => {
    player.tell(
      Text.red("recipe : ")
        .bold(true)
        .append(Text.of(recipe_id.input).aqua().bold(false))
    );
    player.tell(
      Text.red("input : ")
        .bold(true)
        .append(Text.of(items_list).green().bold(false))
    );
    //if(recipe_id.input.length <= items_list.length){
    //recipe_id.input.forEach(e=>{
    player.tell("");
    player.tell(
      items_list.every((value, index) => value === recipe_id.input[index])
    );
  });
